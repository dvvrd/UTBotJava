...
import org.utbot.engine.soot.overrides.UtArrayMock
import org.utbot.engine.soot.overrides.UtLogicMock
import org.utbot.engine.soot.overrides.UtOverrideMock
...
import org.utbot.engine.NotBoolExpression
import org.utbot.engine.UtAddNoOverflowExpression
import org.utbot.engine.UtAddrExpression
import org.utbot.engine.UtAndBoolExpression
import org.utbot.engine.UtArrayApplyForAll
import org.utbot.engine.UtArrayExpressionBase
import org.utbot.engine.UtArraySelectExpression
import org.utbot.engine.UtArraySetRange
import org.utbot.engine.UtArraySort
import org.utbot.engine.UtBoolExpression
import org.utbot.engine.UtBoolOpExpression
import org.utbot.engine.UtBoolSort
import org.utbot.engine.UtBvConst
import org.utbot.engine.UtBvLiteral
import org.utbot.engine.UtByteSort
import org.utbot.engine.UtCastExpression
import org.utbot.engine.UtCharSort
import org.utbot.engine.UtContextInitializer
import org.utbot.engine.UtExpression
import org.utbot.engine.UtFalse
import org.utbot.engine.UtIntSort
import org.utbot.engine.UtIteExpression
import org.utbot.engine.UtLongSort
import org.utbot.engine.UtMkTermArrayExpression
import org.utbot.engine.UtNegExpression
import org.utbot.engine.UtOrBoolExpression
import org.utbot.engine.UtPrimitiveSort
import org.utbot.engine.UtShortSort
import org.utbot.engine.UtSolver
import org.utbot.engine.UtSolverStatusSAT
import org.utbot.engine.UtSubNoOverflowExpression
...
import org.utbot.engine.UtMockInfoGenerator
...
import org.utbot.engine.addrEq
import org.utbot.engine.findTheMostNestedAddr
import org.utbot.engine.isInteger
import org.utbot.engine.mkAnd
import org.utbot.engine.mkBVConst
import org.utbot.engine.mkBoolConst
import org.utbot.engine.mkChar
import org.utbot.engine.mkEq
import org.utbot.engine.mkFalse
import org.utbot.engine.mkFpConst
import org.utbot.engine.mkInt
import org.utbot.engine.mkNot
import org.utbot.engine.mkOr
import org.utbot.engine.select
import org.utbot.engine.simpleMaxSort
import org.utbot.engine.store
...
import org.utbot.engine.asSoftConstraint
import org.utbot.engine.asAssumption
import org.utbot.engine.asUpdate
import org.utbot.engine.asPrimitiveValueOrError
import org.utbot.engine.nullObjectAddr
import org.utbot.engine.soot.util.trusted.isFromTrustedLibrary
import org.utbot.engine.soot.util.statics.concrete.associateEnumSootFieldsWithConcreteValues
import org.utbot.engine.soot.util.statics.concrete.isEnumAffectingExternalStatics
import org.utbot.engine.soot.util.statics.concrete.isEnumValuesFieldName
import org.utbot.engine.soot.util.statics.concrete.makeEnumNonStaticFieldsUpdates
import org.utbot.engine.soot.util.statics.concrete.makeEnumStaticFieldsUpdates
import org.utbot.engine.soot.util.statics.concrete.makeSymbolicValuesFromEnumConcreteValues
...
class Traverser(
     ...
     */
    private fun TraversalContext.arrayStoreExceptionCheck(arrayInstance: ArrayValue, value: SymbolicValue) {
        val valueType = value.type
        val valueBaseType = valueType.baseType
        val arrayElementType = arrayInstance.arrayType.elementType
    ...
    private fun TraversalContext.traverseAssignLeftPart(left: Value, value: SymbolicValue): SymbolicStateUpdate = when (left) {
        is ArrayRef -> {
            ...
            val valueType = value.type
            val valueBaseType = valueType.baseType
            if (valueBaseType is RefType && value is ReferenceValue) {
            ...
            val elementType = arrayInstance.arrayType.elementType
    ...
    fun valueToExpression(value: SymbolicValue, type: Type): UtExpression = when (value) {
        ...
        is PrimitiveValue -> UtCastExpression(value.expr, type.toSort())
    }
    ...
    private fun TraversalContext.resolve(expr: Expr, valueType: Type = expr.type): SymbolicValue =
        when (expr) {
            ...
            is JNegExpr -> UtNegExpression((resolve(expr.op) as PrimitiveValue).expr).toPrimitiveValue(expr.type)
            is JNewExpr -> {
                val addr = findNewAddr()
                val generator = UtMockInfoGenerator { mockAddr ->
    ...
    private fun initStringLiteral(stringWrapper: ObjectValue, value: String) {
        queuedSymbolicStateUpdates += objectUpdate(
            stringWrapper.copy(typeStorage = RefTypeStorage(utStringClass.type)),
            STRING_LENGTH,
            mkInt(value.length)
        )
    ...
    private fun initEnum(type: RefType, addr: UtAddrExpression, ordinal: PrimitiveValue) {
        ...
        val enumValueConstraints = mkOr(
            listOf(addrEq(addr, nullObjectAddr)) + predefinedEnumValues.map {
                mkAnd(
                    addrEq(addr, it.addr),
                    mkEq(ordinal, findEnumOrdinal(it.refType, it.addr))
                )
            }
        )
    ...
    private fun objectInstanceOf(value: ObjectValue, checkType: Type, op: Value): PrimitiveValue {
        ...
        if (value.type.isJavaLangObject() && checkType is ArrayType) {
            ...
            val typeMemoryUpdate = arrayTypeUpdate(value.addr, castedArray.arrayType)
    ...
    private fun TraversalContext.castObject(objectValue: ObjectValue, typeAfterCast: Type, op: Value): SymbolicValue {
        ...
        val currentType = objectValue.refType
...
*/
            ...
            queuedSymbolicStateUpdates += arrayTypeUpdate(array.addr, array.arrayType)
    ...
    private fun TraversalContext.castArray(arrayValue: ArrayValue, typeAfterCast: Type): ArrayValue {
        ...
        if (baseTypeBeforeCast.isJavaLangObject() && baseTypeAfterCast is PrimType) {
            ...
            val memoryUpdate = arrayTypeUpdate(castedArray.addr, castedArray.arrayType)
    ...
    internal fun createNewArray(size: PrimitiveValue, type: ArrayType, elementType: Type): ArrayValue {
        ...
        workaround(HACK) {
            ...
            if (expr is UtBvLiteral) {
                softMaxArraySize = min(HARD_MAX_ARRAY_SIZE, max(expr.value.toInt(), softMaxArraySize))
            }
        }
        queuedSymbolicStateUpdates += Le(length, softMaxArraySize).asHardConstraint() // TODO: fix big array length
        ...
        return ArrayValue(ArrayTypeStorage(type), addr).also {
    ...
    private fun TraversalContext.resolve(
        ...
        is JInstanceFieldRef -> {
            ...
            val concrete = instance.concrete?.value
            val objectType = if (concrete is BaseOverriddenWrapper) {
                concrete.overriddenClass.type
            } else {
        ...
        is JArrayRef -> {
            ...
            val type = arrayInstance.arrayType
    ...
    fun objectUpdate(
        ...
        val descriptor = memoryChunkDescriptor(chunkId, instance.refType, field.type)
    ...
    private fun TraversalContext.traverseCatchBlock(
        ...
        val classId = exception.fold(
            { it.javaClass.id },
            { (exception.symbolic as ObjectValue).refType.id }
        )
    ...
    private fun TraversalContext.mockMakeSymbolic(invokeExpr: JStaticInvokeExpr): List<MethodResult>? {
        ...
        val isNullable = if (parameters.isEmpty()) UtFalse else UtCastExpression(
            (parameters.last() as PrimitiveValue).expr,
            UtBoolSort
        )
    ...
    private fun findInvocationTargets(
        ...
        val methodInvocationTargets = findLibraryTargets(instance.refType, methodSubSignature)?.takeIf {
    ...
    private fun TraversalContext.invoke(
        ...
        when {
            isUtMockAssume || isUtMockAssumeOrExecuteConcretely -> {
                val param = UtCastExpression((parameters.single() as PrimitiveValue).expr, UtBoolSort)
                            ...
                            persistentListOf(
                                simplifiedNamedStore(
                                    ...
                                    UtArrayApplyForAll(memory.findArray(descriptor).select(addr)) { array, i ->
                                        Le(array.select(i).toIntValue(), nullObjectAddr.toIntValue())
                                    }
                                )
                            )
                        )
        ...
        when (target.method.name) {
            utArrayMockArraycopyMethodName -> {
                ...
                val copyValue = UtArraySetRange(
                    ...
                    (parameters[3] as PrimitiveValue).expr,
                    selectArrayExpressionFromMemory(src),
                    (parameters[1] as PrimitiveValue).expr,
                    (parameters[4] as PrimitiveValue).expr
                )
                return listOf(
                    MethodResult(
                        voidValue,
                        memoryUpdates = arrayUpdateWithValue(dst.addr, dst.arrayType, copyValue)
                    )
                )
            }
...
//      TODO: add preferred cex to:  val softConstraints = preferredConstraints(clone)
    ...
    private fun collectSymbolicStateUpdates(expr: UtBoolExpression): SymbolicStateUpdateForResolvedCondition {
        return when (expr) {
            ...
            is UtBoolOpExpression -> {
                ...
                // TODO: JIRA:1667
                val lhs = expr.left as? UtBoolExpression
                    ?: return SymbolicStateUpdateForResolvedCondition()
                val rhsAsIntValue = (expr.right as? UtBvLiteral)?.value?.toInt()
                    ?: return SymbolicStateUpdateForResolvedCondition()
    ...
    private fun lowerIntMulOverflowCheck(
        ...
        val castedLeft = UtCastExpression(left, UtIntSort).toIntValue()
        val castedRight = UtCastExpression(right, UtIntSort).toIntValue()
    ...
    private fun higherIntMulOverflowCheck(
        ...
        val leftValue = toValue(left)
        val rightValue = toValue(right)
    ...
    private fun TraversalContext.intOverflowCheck(op: BinopExpr, leftRaw: PrimitiveValue, rightRaw: PrimitiveValue) {
        ...
        val sort = simpleMaxSort(leftRaw.expr, rightRaw.expr) as UtPrimitiveSort
        val left = leftRaw.expr
        val right = rightRaw.expr

        val overflow = when (op) {
            is JAddExpr -> {
                mkNot(UtAddNoOverflowExpression(left, right))
            }
            is JSubExpr -> {
                mkNot(UtSubNoOverflowExpression(left, right))
            }
            is JMulExpr -> when (sort) {
                is UtByteSort -> lowerIntMulOverflowCheck(left, right, Byte.MIN_VALUE.toInt(), Byte.MAX_VALUE.toInt())
                is UtShortSort -> lowerIntMulOverflowCheck(left, right, Short.MIN_VALUE.toInt(), Short.MAX_VALUE.toInt())
                is UtIntSort -> higherIntMulOverflowCheck(left, right, Int.SIZE_BITS, Int.MIN_VALUE.toLong()) { it: UtExpression -> it.toIntValue() }
                is UtLongSort -> higherIntMulOverflowCheck(left, right, Long.SIZE_BITS, Long.MIN_VALUE) { it: UtExpression -> it.toLongValue() }
                else -> null
            }
            else -> null
        }
        ...
        if (overflow != null) {
            implicitlyThrowException(ArithmeticException("${left.sort} ${op.symbol} overflow"), setOf(overflow))
    ...
    private fun TraversalContext.classCastExceptionCheck(valueToCast: ReferenceValue, typeAfterCast: Type) {
        ...
        if (baseTypeAfterCast is RefType) {
            ...
            val internal = newAddr.internal
            val parameterizedType = when (internal) {
                is UtArraySelectExpression -> parameterAddrToGenericType[findTheMostNestedAddr(internal)]

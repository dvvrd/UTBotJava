...
//            .map { (if (it is ArrayType) it.baseType else it) to it.numDimensions }
            .filterNotTo(mutableSetOf()) {
                if (it is ArrayType)
                    isInappropriateOrArrayOfMocksOrLocals(it.numDimensions, it.baseType)
                else
                    isInappropriateOrArrayOfMocksOrLocals(0, it)
            }
...
//            }
    ...
    fun nullObject(type: Type) = when (type) {
        is RefType, is NullType, is VoidType -> ObjectValue(RefTypeStorage(type), nullObjectAddr)
        is ArrayType -> ArrayValue(ArrayTypeStorage(type), nullObjectAddr)

...
import org.utbot.engine.soot.TypeRegistry.Companion.objectNumDimensions
...
import org.utbot.engine.UtInt32Sort
...
class Resolver(
    ...
    private fun resolveReferenceValue(value: ReferenceValue): UtModel {
        ...
        val constructedType = holder.constructTypeOrNull(value.addr, value.type) ?: return UtNullModel(value.refType.id)
    ...
    private fun resolveObject(objectValue: ObjectValue): UtModel {
        ...
        if (concreteAddr == SYMBOLIC_NULL_ADDR) {
            return UtNullModel(objectValue.refType.sootClass.id)
        }
        ...
        val constructedType = holder.constructTypeOrNull(objectValue.addr, objectValue.type) as? RefType
            ?: return UtNullModel(objectValue.refType.id)
        val concreteType =
            typeResolver.findAnyConcreteInheritorIncludingOrDefaultUnsafe(constructedType, objectValue.refType)

        resolveWrappersAndOverriddenTypesOrNull(concreteType, objectValue.addr, objectValue.type)?.let { return it }

        return constructModel(objectValue.addr, objectValue.refType, concreteType)
    }
    ...
    private fun constructArrayModel(instance: ArrayValue): UtModel {
        ...
        if (concreteAddr == SYMBOLIC_NULL_ADDR) {
            return UtNullModel(instance.arrayType.id)
        }

        val evaluatedType = workaround(HACK) { memory.findTypeForArrayOrNull(instance.addr) as? ArrayType ?: instance.arrayType }

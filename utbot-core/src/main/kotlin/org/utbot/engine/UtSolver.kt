...
import org.utbot.common.prettify
import org.utbot.engine.Eq
import org.utbot.engine.PrimitiveValue
import org.utbot.engine.UtSolverStatusKind.SAT
import org.utbot.engine.UtSolverStatusKind.UNKNOWN
import org.utbot.engine.UtSolverStatusKind.UNSAT
...
open class VisitorFactory {
    open fun rewritingVisitor(constraints: Set<UtBoolExpression> = emptySet(),
                              eqs: Map<UtExpression, UtExpression> = emptyMap(),
                              lts: Map<UtExpression, Long> = emptyMap(),
                              gts: Map<UtExpression, Long> = emptyMap()) = RewritingVisitor(constraints, eqs, lts, gts)
    open fun axiomInstantiationVisitor(constraints: Set<UtBoolExpression> = emptySet(),
                                       eqs: Map<UtExpression, UtExpression> = emptyMap(),
                                       lts: Map<UtExpression, Long> = emptyMap(),
                                       gts: Map<UtExpression, Long> = emptyMap()) =
        AxiomInstantiationRewritingVisitor(constraints, eqs, lts, gts)
    open fun z3TranslatorFactory(z3Context: Context) = Z3TranslatorVisitor(z3Context)
}

data class UtSolver constructor(
    private val visitors: VisitorFactory,
    ...
    private var constraints: BaseQuery = Query(visitors),
    ...
    private val translator: Z3TranslatorVisitor = visitors.z3TranslatorFactory(context)

    /**
     * Constraints from the [assumption] that are not satisfiable.
     * All the elements were found in the calculated unsat core.
     */
    val failedAssumptions = mutableListOf<UtBoolExpression>()
    ...
    constructor(visitors: VisitorFactory, trackableResources: MutableSet<AutoCloseable>, timeout: Int) : this(
        visitors
    ) {

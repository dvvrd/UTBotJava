...
open class Z3TranslatorVisitor(private val z3Context: Context) : UtExpressionVisitor<Expr> {
    ...
    override fun visit(expr: UtOpExpression): Expr = expr.run {
        val leftResolve = translate(left).z3Variable(left.sort)
        val rightResolve = translate(right).z3Variable(right.sort)
    ...
    override fun visit(expr: UtNegExpression): Expr = expr.run {
        negate(z3Context, translate(variable).z3Variable(variable.sort))
    }

    override fun visit(expr: UtCastExpression): Expr = expr.run {
        val z3var = translate(variable).z3Variable(variable.sort)
        z3Context.convertVar(z3var, sort).expr
    }
    ...
    override fun visit(expr: UtStringCharAt): Expr = expr.run {
        ...
        z3Context.convertVar(z3var, UtCharSort).expr
    }

...
data class Query(
    ...
    val rewriter: RewritingVisitor
        get() = visitors.rewritingVisitor(hard, eqs, lts, gts)
    ...
    private fun Collection<UtBoolExpression>.simplify(
        ...
        visitors.axiomInstantiationVisitor(hard, eqs, lts, gts).let { visitor ->
        ...
        for (expr in addedHard) {
            when (expr) {
                ...
                is UtBoolOpExpression -> when (expr.operator) {
                    ...
                    Lt -> if (expr.right.isConcrete && expr.right.isInteger()) {
                        val rightValue = (expr.right.toConcrete() as Number).toLong()
                        if (rightValue == Long.MIN_VALUE) {
                            return UnsatQuery(this.hard.addAll(addedHard))
                        }
                        // decrease upper bound
                        val leftExpr = if (expr.left is UtAddrExpression) expr.left.internal else expr.left
                        addedLts.merge(leftExpr, rightValue - 1, ::min)
                    }
                    Le -> if (expr.right.isConcrete && expr.right.isInteger()) {
                        val rightValue = (expr.right.toConcrete() as Number).toLong()
                        // decrease upper bound
                        val leftExpr = if (expr.left is UtAddrExpression) expr.left.internal else expr.left
                        addedLts.merge(leftExpr, rightValue, ::min)
                    }
                    Gt -> if (expr.right.isConcrete && expr.right.isInteger()) {
                        val rightValue = (expr.right.toConcrete() as Number).toLong()
                    ...
                    Ge -> if (expr.right.isConcrete && expr.right.isInteger()) {
                        val rightValue = (expr.right.toConcrete() as Number).toLong()
                        // increase lower bound
                        val leftExpr = if (expr.left is UtAddrExpression) expr.left.internal else expr.left

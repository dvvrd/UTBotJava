...
open class Z3EvaluatorVisitor(private val model: Model, private val translator: Z3TranslatorVisitor) :
    ...
    override fun visit(expr: UtArrayShiftIndexes): Expr = expr.run {
        val lastIndex = selectIndexStack.last().intValue()
        val offset = eval(offset).intValue()
    ...
    override fun visit(expr: UtOpExpression): Expr = expr.run {
        val leftResolve = eval(left).z3Variable(left.sort)
        val rightResolve = eval(right).z3Variable(right.sort)
    ...
    override fun visit(expr: UtNegExpression): Expr = expr.run {
        translator.withContext {
            negate(this, eval(variable).z3Variable(variable.sort))
        }
    }

    override fun visit(expr: UtCastExpression): Expr = expr.run {
        val z3var = eval(variable).z3Variable(variable.sort)
        translator.withContext { convertVar(z3var, sort).expr }
    }

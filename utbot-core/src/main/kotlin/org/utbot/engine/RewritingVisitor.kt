...
open class RewritingVisitor(
    protected val constraints: Set<UtBoolExpression> = emptySet(),
    protected val eqs: Map<UtExpression, UtExpression> = emptyMap(),
    protected val lts: Map<UtExpression, Long> = emptyMap(),
    protected val gts: Map<UtExpression, Long> = emptyMap()
) : UtExpressionVisitor<UtExpression> {
    val axiomInstantiationVisitor: RewritingVisitor
        get() = AxiomInstantiationRewritingVisitor(constraints, eqs, lts, gts)
    protected val selectIndexStack = mutableListOf<UtExpression>()
    protected val simplificationCache = IdentityHashMap<UtExpression, UtExpression>()
    ...
    protected fun checkSortsEquality(value: () -> UtExpression, sourceExpr: UtExpression): UtExpression {
        val result = value()
        ...
        return UtCastExpression(result, sourceSort)
    }
        ...
        applySimplification(expr) {
            ...
            when (expr.operator) {
                ...
                Add -> when {
                    ...
                    left.isIntegralLiteral -> Add(right, left).accept(this)
                    ...
                    right.isIntegralLiteral && left is UtOpExpression && left.operator is Add && left.right.isIntegralLiteral ->
                        Add(
                            left.left,
                            evalIntegralLiterals(left.right, right, expr.right.sort, Long::plus)
                        )
                    else -> Add(left, right)
                }
                // CONFLUENCE:UtBot+Expression+Optimizations#UtBotExpressionOptimizations-subToAdd
                // Sub(a, b) ---> Add(a, Neg b)
                Sub -> Add(left, UtNegExpression(right)).accept(this)
                Mul -> when {
                    ...
                    right.isIntegralLiteral -> when {
                        ...
                        right.toLong() == -1L -> UtNegExpression(left)

                        // Mul(Op(_, Integral _), Integral _)
                        left is UtOpExpression && left.right.isIntegralLiteral -> when (left.operator) {
                            ...
                            is Add -> Add(
                                Mul(left.left, right),
                                evalIntegralLiterals(left.right, right, expr.resultSort, Long::times)
                            )
                            ...
                            is Mul -> Mul(
                                left.left,
                                evalIntegralLiterals(left.right, right, expr.right.sort, Long::times)
                            )
                            else -> Mul(left, right)
                        }
                        else -> Mul(left, right)
                    ...
                    left.isIntegralLiteral -> Mul(right, left).accept(this)
                    else -> Mul(left, right)
                }
                Div -> when {
                    ...
                    right.isIntegralLiteral && right.toLong() == -1L -> UtNegExpression(left)
                    else -> Div(left, right)
                }
                And -> when {
                    ...
                    left.isIntegralLiteral -> And(right, left)
                    else -> And(left, right)
                }
                Cmp, Cmpg, Cmpl -> when {
                ...
                Or -> when {
                    ...
                    left.isIntegralLiteral -> Or(right, left)
                    else -> Or(left, right)
                }
                Shl -> when {
                ...
                Xor -> when {
                    ...
                    // Xor(a, a) ---> Integral 0
                    left == right -> (0L).toValueWithSort(expr.resultSort).primitiveToLiteral()
                    else -> Xor(left, right)
                }
                Rem -> when {
        ...
        applySimplification(expr) {
            when (val exp = expr.expr.accept(this) as UtBoolExpression) {
                ...
                is UtBoolOpExpression -> {
                    val left = exp.left
                    val right = exp.right
                    if (left.sort is UtBvSort && right.sort is UtBvSort) {
                        when (exp.operator) {
                            // not Ne(a, b) ---> Eq(a, b)
                            Ne -> Eq(left, right)
                            // not Ge(a, b) ---> Lt(a, b)
    ...
    override fun visit(expr: UtNegExpression): UtExpression =
        applySimplification(expr) {
            val variable = expr.variable.accept(this)
            if (variable.isConcrete) {
                ...
                when (variable.sort) {
                    ...
                    else -> UtNegExpression(variable)
                }
            } else {
                UtNegExpression(variable)
            }
        }

    override fun visit(expr: UtCastExpression): UtExpression =
        applySimplification(expr) {
            val newExpr = expr.variable.accept(this)
            when {
                // Cast(a@(PrimitiveValue type), type) ---> a
                expr.sort == expr.variable.sort -> newExpr
                newExpr.isConcrete -> when (newExpr) {
                    is UtBoolLiteral -> newExpr.toLong().toValueWithSort(expr.sort).primitiveToLiteral()
                    is UtBvLiteral -> newExpr.toLong().toValueWithSort(expr.sort).primitiveToLiteral()
                    is UtFpLiteral -> newExpr.value.toValueWithSort(expr.sort).primitiveToLiteral()
                    else -> UtCastExpression(newExpr, expr.sort)
                }
                else -> UtCastExpression(newExpr, expr.sort)
            }
        }
     ...
     */
    private fun simplifyGreater(left: UtExpression, right: UtExpression, including: Boolean): UtExpression {
        val leftExpr = if (left is UtAddrExpression) left.internal else left
        val lowerBound = gts[leftExpr]
        val upperBound = lts[leftExpr]
        ...
        return if (lowerBound == null && upperBound == null) {
            ...
            val rightValue = right.toLong() + if (including) 0 else 1
            when {
                // left > Long.MAX_VALUE
                ...
                upperBound != null && rightValue - upperBound == 0L -> Eq(
                    left,
                    rightValue.toValueWithSort(right.sort).primitiveToLiteral()
                )
     ...
     */
    private fun simplifyLess(left: UtExpression, right: UtExpression, including: Boolean): UtExpression {
        val leftExpr = if (left is UtAddrExpression) left.internal else left
        val lowerBound = gts[leftExpr]
        val upperBound = lts[leftExpr]
    ...
    private fun maxSort(left: UtSort, right: UtSort): UtSort = when {
        left is UtLongSort -> left
        right is UtLongSort -> right
        else -> UtIntSort
    }
        ...
        applySimplification(expr) {
            ...
            when (expr.operator) {
                Eq -> when {
                    ...
                    right.isIntegralLiteral && left is UtOpExpression && left.right.isIntegralLiteral -> when (left.operator) {
                        ...
                        Xor -> Eq(
                            left.left,
                            evalIntegralLiterals(right, left.right, maxSort(expr.right, left.right), Long::xor)
                        )
                        else -> Eq(left, right)
                    }
                    /**
                    ...
                    // Eq(a@(Concrete _), b) ---> Eq(b, a)
                    left.isConcrete -> Eq(right, left).accept(this)
                    else -> Eq(left, right)
                }
                ...
                Le -> when {
                     ...
                     * @see simplifyGreater
                     */
                    right.isIntegralLiteral -> simplifyLess(left, right, true)
                    left.isIntegralLiteral -> simplifyGreater(right, left, true)
                    else -> Le(left, right)
                }
                Lt -> when {
                     ...
                     * @see simplifyGreater
                     */
                    right.isIntegralLiteral -> simplifyLess(left, right, false)
                    left.isIntegralLiteral -> simplifyGreater(right, left, false)
                    else -> Lt(left, right)
                }
                Ge -> when {
                     ...
                     * @see simplifyGreater
                     */
                    right.isIntegralLiteral -> simplifyGreater(left, right, true)
                    left.isIntegralLiteral -> simplifyLess(right, left, true)
                    else -> Ge(left, right)
                }
                Gt -> when {
                     ...
                     * @see simplifyGreater
                     */
                    right.isIntegralLiteral -> simplifyGreater(left, right, false)
                    left.isIntegralLiteral -> simplifyLess(right, left, false)
                    else -> Gt(left, right)
                }
            }
        }
    ...
    override fun visit(expr: UtStringLength): UtExpression =
        applySimplification(expr) {
            ...
            when {
                string is UtArrayToString -> string.length
    ...
    override fun visit(expr: UtStringCharAt): UtExpression =
        applySimplification(expr) {
            ...
            if (allConcrete(string, index)) {
                (string.toConcrete() as String)[index.toConcrete() as Int].primitiveToLiteral()
            } else {
...
class AxiomInstantiationRewritingVisitor(
...
) : RewritingVisitor(constraints, eqs, lts, gts) {
    ...
    override fun visit(expr: UtArrayInsert): UtExpression {
        ...
        instantiatedAxiomExpressions += UtEqExpression(
            ...
            ).accept(this), arrayInstance.select(selectedIndex)
        )
    ...
    override fun visit(expr: UtArrayInsertRange): UtExpression {
        val arrayInstance = instantiateArrayAsNewConst(expr)
        val index = expr.index.accept(this)
        val from = expr.from.accept(this)
        val length = expr.length.accept(this)
        val selectedIndex = selectIndexStack.last()
        val pushedArrayInstanceIndex =
            UtIteExpression(Lt(selectedIndex, index), selectedIndex, Sub(selectedIndex, length))
        val arrayExpression = withNewSelect(pushedArrayInstanceIndex) { expr.arrayExpression.accept(this) }
        val pushedElementsIndex = Add(Sub(selectedIndex, index), from)
        val elements = withNewSelect(pushedElementsIndex) { expr.elements.accept(this) }
        instantiatedAxiomExpressions += UtEqExpression(
            UtIteExpression(
                mkAnd(Ge(selectedIndex, index), Lt(selectedIndex, Add(index, length))),
    ...
    override fun visit(expr: UtArrayRemove): UtExpression {
        ...
        instantiatedAxiomExpressions += UtEqExpression(
            arrayExpression.select(pushedIndex),
            arrayInstance.select(selectedIndex)
        )
        return arrayInstance
    }
    ...
    override fun visit(expr: UtArrayShiftIndexes): UtExpression {
        val arrayInstance = instantiateArrayAsNewConst(expr)
        val offset = expr.offset.accept(this)
        val selectedIndex = selectIndexStack.last()
    ...
    override fun visit(expr: UtArrayToString): UtExpression {
        ...
        instantiatedAxiomExpressions += UtEqExpression(
            arrayExpression.select(pushedIndex),
            UtStringCharAt(stringInstance, selectedIndex)
        )
